...
#############################################################################################################
tickers = ["NSE:ABFRL", #1000 ... 5 qty
           "NSE:ADANIENT", #rs 100... 50 qty
           "NSE:INFY"]


#3 inside bars
capital = 5000 #position size  $60  =5000/60 = 85
    ...
    if dt1.second <= 1 and dt1.minute % timeFrame == 0:
        ...
        for ticker in tickers:
            ...
            try:
                ...
                #check if inside bar is made
                if indicator_dir[ticker][0] == 0:
                    if high[-1] < high[-2] and high[-2] < high[-3] and low[-1]>low[-2] and low[-2] > low[-3]:
                        indicator_dir[ticker][0] = high[-1]
                        indicator_dir[ticker][1] = low[-1]

                #check for BUY ENTRY
                if (number_of_trades < maxtrades) and (indicator_dir[ticker][2] == 0) and (close[-1]>indicator_dir[ticker][0]) and (indicator_dir[ticker][0] != 0):
                ...
                elif (number_of_trades < maxtrades) and (indicator_dir[ticker][2] == 0) and (close[-1]<indicator_dir[ticker][1]) and (indicator_dir[ticker][0] != 0):
