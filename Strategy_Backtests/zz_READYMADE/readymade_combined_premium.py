...
def findStrikePriceATM():
    ...
    if stock == "BANKNIFTY":
        ...
        closest_Strike = int(round((ltp / 50),0) * 50)  #  22519.4/50 = 450.388 = 450 = 22500
        print(closest_Strike)
...
def findStrikePricePremium():
    ...
    for strike in strikeList:
        ltp_option = findManualPrice(helper.getOptionFormat(stock,intExpiry,strike,"CE"))
        print(ltp_option)
    ...
    prev_diff = 10000
    for strike in strikeList:
        ltp_option = findManualPrice(helper.getOptionFormat(stock,intExpiry,strike,"PE"))
        diff = abs(ltp_option - premium)
        print("diff==>", diff)
        if (diff < prev_diff):
...
def takeEntry(closest_Strike_CE, closest_Strike_PE, atmCE, atmPE):
    ...
    #ceSL = round((ce_entry_price + pe_entry_price) + SL_point, 1)
    #peSL = round(pe_entry_price + SL_point, 1)
    #ceTarget = round((ce_entry_price + pe_entry_price) - target_point, 1)
    #peTarget = round(pe_entry_price - target_point, 1)
    ceSL = round((ce_entry_price + pe_entry_price) * (1 + SL_percentage / 100), 1)   #150 x (1.1) = 165
    peSL = round(pe_entry_price * (1 + SL_percentage / 100), 1)
    ceTarget = round((ce_entry_price + pe_entry_price) * (1 - target_percentage / 100), 1)
...
def exitPosition(atmCE, ceSL, ceTarget, ce_entry_price, atmPE, peSL, peTarget, pe_entry_price, qty):
    ...
    while traded == "No":
        ...
        try:
            ...
            if ((ltp + ltp1 > ceSL) or (ltp + ltp1 < ceTarget) or (dt.hour >= 15 and dt.minute >= 10)) and ltp != -1:
                ...
                df["PnL"] = [PnL]
                print("The OID of Exit CE and PE is: ", oidexitCE, oidexitPE)
                traded = "Close"
