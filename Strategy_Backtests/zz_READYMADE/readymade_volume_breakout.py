    ...
    if dt1.second < 3 and dt1.minute % timeFrame == 0:
        ...
        for ticker in tickers:
            ...
            try:
                ...
                print(data)
                print(data[['volume']].to_string(index=True))
                ...
                #check if volume breakout is done
                if indicator_dir[ticker][0] == 0:
                    print("here")
                    print(close[-1])
                    print(opens[-1])
                    if volume[-2] > volume_level and close[-1] > opens[-1]:
                        indicator_dir[ticker][0] = high[-1]
                        indicator_dir[ticker][1] = -1
                    elif volume[-2] > volume_level and close[-1] < opens[-1]:
                        indicator_dir[ticker][0] = -1
                        indicator_dir[ticker][1] = low[-1]

                #check for BUY ENTRY
                if (indicator_dir[ticker][2] == 0) and (close[-1]>indicator_dir[ticker][0]) and (indicator_dir[ticker][0] > 0):
                ...
                elif (indicator_dir[ticker][2] == 0) and (close[-1]<indicator_dir[ticker][1]) and (indicator_dir[ticker][1] > 0):
