...
def findStrikePriceATM(stock, cepe):
    ...
    if (cepe == "CE"):
        ...
        oidentry = placeOrder1( atmCE, "SELL", qty, "MARKET", 0, "regular",papertrading)
        tradeCEoption = atmCE
        print("CE Entry OID: ", oidentry)
    else:
        #SELL AT MARKET PRICE
        print("In PE placeorder")
        oidentry = placeOrder1( atmPE, "SELL", qty, "MARKET", 0, "regular",papertrading)
        tradePEoption = atmPE
...
def exitPosition(tradeOption):
    ...
    oidentry = placeOrder1( tradeOption, "BUY", qty, "MARKET", 0, "regular",papertrading)
...
while x == 1:
    ...
    if dt1.second <= 1 and dt1.minute % timeFrame == 0:
        ...
        if st==0:
            ...
            if close[-1]>supertrend_val and close[-2] < supertrend_val_prev:
            ...
            elif close[-1]<supertrend_val and close[-2] > supertrend_val_prev:
                sl=float(close[-1])+sl_point
                target=float(close[-1])-target_point
                st=2
