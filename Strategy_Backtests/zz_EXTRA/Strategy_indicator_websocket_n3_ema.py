...
while x == 1:
    ...
    if dt1.second <= 1 and dt1.minute % timeFrame == 0:
        ...
        #crossover
        ema_20 = ta.trend.EMAIndicator(pd.Series(close),20,False).ema_indicator().iloc[-1]
        ema_50 = ta.trend.EMAIndicator(pd.Series(close),50,False).ema_indicator().iloc[-1]
        ema_20_p = ta.trend.EMAIndicator(pd.Series(close),20,False).ema_indicator().iloc[-2]
        ema_50_p = ta.trend.EMAIndicator(pd.Series(close),50,False).ema_indicator().iloc[-2]
        ...
        print("sma 20", ema_20)
        print("sma 50", ema_50)
        print("sma 20 p", ema_20_p)
        print("sma 50 p", ema_50_p)
        ...
        if st==0:
            if (ema_20>ema_50 and ema_20_p<ema_50_p):
            ...
            elif (ema_20<ema_50 and ema_20_p>ema_50_p):
                sl=float(close[-1])+sl_point
                target=float(close[-1])-target_point
                st=2
