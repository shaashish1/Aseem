...
def subscribeFeeds(breeze):
    ...
    for option_stock in optInstrList:
        ...
        if opt_search:
            ...
            print(right)
            right = "Put" if right == "PE" else "Call"
...
def main():
    ...
    while True:
        ...
        if not connected:
            ...
            test_data = requests.get('http://localhost:4000/ltp?instrument=NSE:NIFTY50').json()
            print(test_data)
            if(x==test_data):
                count_same_x += 1
                if(count_same_x==15):
