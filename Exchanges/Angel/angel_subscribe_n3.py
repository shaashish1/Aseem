...
a = helper.getHistorical("NSE:NIFTY",1,1)
...
def connectFeed(SmartWebSocketV2):
    ...
    # SmartWebSocketV2.connect()
    threading.Thread(target = SmartWebSocketV2.connect).start()
    connected = True
    reconnect = False
    x = requests.get('http://localhost:4000/ltp?instrument=NSE:NIFTY').json()
    print(x)
    count_same_x=0
    while True:
        current_time = dt.datetime.now().time()
        if current_time >= end_time:
            SmartWebSocketV2.close_connection()
            break
        if not connected:
            ...
            test_data = requests.get('http://localhost:4000/ltp?instrument=NSE:NIFTY').json()
            print(test_data)
            if(x==test_data):
                count_same_x += 1
                if(count_same_x==15):
